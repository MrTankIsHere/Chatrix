...
import { Search, X, Edit2, Check, Wallet } from 'lucide-react';
import { validateSolanaAddress, setPeerNickname } from '../lib/peers';
import { fetchProfile } from '../lib/profile';
// Removed Solana Name Service lookups
...
export const RecipientInput: React.FC<Props> = ({ onRecipientSelect, recentPeers }) => {
  ...
  const [inputType, setInputType] = useState<'address' | null>(null);
  ...
  const validateAndSelectAddress = async (value: string) => {
    ...
    try {
      // Direct address input only
      const isValid = await validateSolanaAddress(value);
        if (isValid) {
          ...
          try {
            const profile = await fetchProfile(value);
      ...
      <div className="relative">
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          {inputType && !isValidating && (
            <div className="p-1 rounded-lg bg-card-highlight" title={'Wallet Address'}>
